...
const useVotingPowerDetails = () => {
  const { vaultBalance, miniMeTokenBalance, account, miniMeToken, depositToken } = useAppState()

  return useMemo(() => {
    const votingPower =
      vaultBalance && !vaultBalance.isEqualTo(0) ? parseFloat(miniMeTokenBalance.dividedBy(vaultBalance)) : 0
    ...
    return [
      {
        ...
        miniMeTokenBalance:
          miniMeTokenBalance && account
            ...
            : '-',
        vaultBalance: vaultBalance ? strip(offChainFormat(vaultBalance, depositToken.decimals).toString()) : '-',
      },
    ]
